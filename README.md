...
9. **R2 uploads (registration, profile photos):** The app expects `R2_ACCOUNT_ID`, `R2_BUCKET_NAME`, `R2_PUBLIC_URL` as plaintext **vars** and `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY` as **Secrets**, all on **this** Worker. If variables exist in the dashboard but uploads still fail, add the same keys to **`wrangler.jsonc`** (`vars` for plaintext) and `wrangler secret put` for keys so CLI deploys match the dashboard. `lib/storage.ts` resolves values from `process.env` and from the Worker `env` object (OpenNext).
